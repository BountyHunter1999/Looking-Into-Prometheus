run_first_alert:
	...
	docker run --rm -d -p 9090:9090 --network prom --name prometheus -v $$(pwd)/configs/prometheus.yml:/etc/prometheus/prometheus.yml -v $$(pwd)/configs/rules.yml:/etc/prometheus/rules.yml prom/prometheus:v2.53.0
	docker run --rm -d -p 9093:9093 --network prom --name alert-manager -v $$(pwd)/configs/alert-manager.yml:/etc/alertmanager/alertmanager.yml prom/alertmanager:main
